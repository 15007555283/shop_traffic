        ...
        for i in range(20):
            for item in self.activeList:
                ...
                try:
                    ...
                    proxy_list = self.proxy_list[proxy_num - 1].split(':')
                    proxy_obj = {'ip': proxy_list[0], 'port': proxy_list[1]}
                    del self.proxy_list[proxy_num]
                    chrome_options.add_argument(f"--proxy-server=http://{proxy_obj['ip']}:{proxy_obj['port']}")
